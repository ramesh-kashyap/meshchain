...
import { createWithdrawal, getWithdrawalHistory } from "../../Requests/Api";  // Fix import

const networkOptions = [
  { name: "Binance Smart Chain", logo: "upnl/assets/icons/icon_bnb_3.svg" },
  { name: "USDT", logo: "upnl/assets/icons/logo_usdt_2.svg" },
];
...
const WithdrawComponent = () => {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const data = await getWithdrawalHistory(1);  // Replace with actual user_id_fk
        setWithdrawals(data);
      } catch (error) {
  ...
  const handleWithdraw = async () => {
    try {
      ...
      const response = await  createWithdrawal(address, amount); // Replace 1 with actual user ID
      alert(response.message || "Withdrawal successful");
      setWithdrawals([response.withdrawal, ...withdrawals]);  // Update withdrawals list
        ...
        <div className="lg:col-span-3 xl:col-span-2 bg-white rounded-[16px] p-6">
          ...
          <div className="w-full flex flex-col gap-3 md:gap-5">
            ...
            <div className="flex flex-col gap-2">
              ...
              />
              <div className="font-semibold cursor-pointer" style={{ color: "#32CD32", fontWeight: "600" }}>
                Max
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 w-full flex justify-end">
            <button onClick={handleWithdraw} className="bg-green-500 text-white px-3 py-1 rounded-[30px] md:w-auto disabled:opacity-50">
              Confirm
            </button>
          </div>
        </div>
            ...
            {withdrawals.length > 0 ? (
              ...
              <div className="flex h-full justify-center items-center w-full text-secondary">
                <div className="w-full text-center">
                  <img alt="Icon Empty" loading="lazy" width="64" height="40" style={{width:"60px"}} className="mx-auto mb-2" src="upnl/assets/icons/empty_state.svg" />
