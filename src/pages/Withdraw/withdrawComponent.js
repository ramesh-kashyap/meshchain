  ...
  const [withdrawals, setWithdrawals] = useState([]); // Store withdrawals data
    ...
    useEffect(() => {
      const fetchWithdrawalHistory = async () => {
        ...
        console.log("Tokenddd:", token); // Check if the token is retrieved
        ...
        {withdrawals.map((withdrawal, index) => (
          <div key={index} className="flex justify-between items-center py-2 border-b">
            ...
            <p className="font-medium">withdraw</p>
              ...
              <p className="font-medium text-sm text-gray-600">{withdrawal.created_at}</p>
            </div>
            <div className="w-1/2 text-right">
              <p className="text-green-500"><span>-{withdrawal.amount}</span></p>
            </div>
          </div>
        ))}
      </>
